...
void buildMaxHeap(BinaryHeap* BH){
    ...
    for(int i = (BH->count/2)-1; i >= 0; i--){
        ...
        while((2*count)+1 < BH->count || (2*count)+2 < BH->count){
            ...
            } else if((2 * count) + 1 < BH->count){ // left
...
void maxHeapSort(BinaryHeap* BH){
    // buildMaxHeap(BH); // must be a max heap already
...
void deleteBH(BinaryHeap* BH){
    ...
    BH->count--;
    int num = BH->elems[0];
    int left, right, child, childIndx;
    int i = 0;

    while((2*i)+1 < BH->count-1 || (2*i)+2 < BH->count-1){
